# cdn.iamrp.dev Asset Directory

This document provides a machine-readable directory of all available assets hosted on `cdn.iamrp.dev`. 
**Base URL:** `https://cdn.iamrp.dev/`

## JavaScript Libraries (`/js/`)

### Core & Frameworks
- `js/jquery/jquery-3.7.1.min.js`
- `js/react/react.production.min.js`
- `js/react/react-dom.production.min.js`
- `js/vue/vue.global.prod.js`
- `js/alpine/alpine.min.js`

### Utilities
- `js/axios/axios.min.js`
- `js/lodash/lodash.min.js`

### UI & Styling Scripts
- `js/bootstrap/bootstrap.bundle.min.js`
- `js/tailwindcss/tailwindcss.js`

### Other Scripts
- `js/d3.min.js`
- `js/pixi.js`
- `js/AddPageApp.js`
- `js/ViewPageApp.js`
- `js/1x1-00000000.png`

## CSS Frameworks & Styles (`/css/`)

### Frameworks
- `css/bootstrap/bootstrap.min.css`

### Fonts & Icons CSS
- `css/fontawesome/all.min.css`
- `css/inter/inter.css`
- `css/roboto/roboto-400.css`
- `css/roboto/roboto-700.css`

### Other CSS
- `css/1x1-00000000.png`

## Fonts & Icons (`/fonts/`)

### FontAwesome Webfonts
- `fonts/fontawesome/webfonts/fa-brands-400.woff2`
- `fonts/fontawesome/webfonts/fa-regular-400.woff2`
- `fonts/fontawesome/webfonts/fa-solid-900.woff2`
- `fonts/fontawesome/webfonts/fa-v4compat.woff2`

### Inter Font
- `fonts/inter/inter-latin-wght-normal.woff2`

### Roboto Font
- `fonts/roboto/roboto-latin-400-normal.woff2`
- `fonts/roboto/roboto-latin-700-normal.woff2`

### JetBrains Mono
- `fonts/jetbrains-mono/JetBrainsMono-Bold.woff2`
- `fonts/jetbrains-mono/JetBrainsMono-BoldItalic.woff2`
- `fonts/jetbrains-mono/JetBrainsMono-ExtraBold.woff2`
- `fonts/jetbrains-mono/JetBrainsMono-ExtraBoldItalic.woff2`
- `fonts/jetbrains-mono/JetBrainsMono-ExtraLight.woff2`
- `fonts/jetbrains-mono/JetBrainsMono-ExtraLightItalic.woff2`
- `fonts/jetbrains-mono/JetBrainsMono-Italic.woff2`
- `fonts/jetbrains-mono/JetBrainsMono-Italic[wght].woff2`
- `fonts/jetbrains-mono/JetBrainsMono-Light.woff2`
- `fonts/jetbrains-mono/JetBrainsMono-LightItalic.woff2`
- `fonts/jetbrains-mono/JetBrainsMono-Medium.woff2`
- `fonts/jetbrains-mono/JetBrainsMono-MediumItalic.woff2`
- `fonts/jetbrains-mono/JetBrainsMono-Regular.woff2`
- `fonts/jetbrains-mono/JetBrainsMono-SemiBold.woff2`
- `fonts/jetbrains-mono/JetBrainsMono-SemiBoldItalic.woff2`
- `fonts/jetbrains-mono/JetBrainsMono-Thin.woff2`
- `fonts/jetbrains-mono/JetBrainsMono-ThinItalic.woff2`
- `fonts/jetbrains-mono/JetBrainsMono[wght].woff2`

## Images (`/images/`)
- `images/avatars/1x1-00000000.png`
- `images/blog/1x1-00000000.png`
- `images/hardware/1x1-00000000.png`
- `images/security/1x1-00000000.png`

## System Files
- `CNAME`
- `README.md`
- `index.md`
